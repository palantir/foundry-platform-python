...
class ParameterEvaluationResult(BaseModel):
    ...
    evaluated_constraints: Optional[List[ParameterEvaluatedConstraint]] = Field(
        alias="evaluatedConstraints", default=None
    )

    required: StrictBool = Field()
    """Represents whether the parameter is a required input to the action."""

    _properties: ClassVar[Set[str]] = set(["result", "evaluatedConstraints", "required"])
    ...
    @classmethod
    def from_dict(cls, obj: Dict, *, allow_extra=False) -> "ParameterEvaluationResult":
