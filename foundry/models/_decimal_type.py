...
class DecimalType(BaseModel):
    ...
    precision: Optional[StrictInt] = Field(default=None)

    scale: Optional[StrictInt] = Field(default=None)

    type: Literal["decimal"] = Field()

    _properties: ClassVar[Set[str]] = set(["precision", "scale", "type"])
    ...
    @classmethod
    def from_dict(cls, obj: Dict, *, allow_extra=False) -> "DecimalType":
