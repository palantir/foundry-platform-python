...
class ApiClient:
    ...
    def _serialize(self, value: Any, value_type: Any) -> Optional[bytes]:
        ...
        if value_type is Any:
            ...
            self._ensure_type_adapter(value_type)
            ...
            json_bytes: bytes = value_type.type_adapter.dump_json(
                value, exclude_unset=True, by_alias=True
            )
    ...
    def _deserialize(self, res: Response, response_type: Any) -> Any:
        ...
        self._ensure_type_adapter(response_type)
        return response_type.type_adapter.validate_python(data)

    @staticmethod
    def _ensure_type_adapter(_type: Any):
        if hasattr(_type, "type_adapter"):
            return

        if isclass(_type) and issubclass(_type, BaseModel):
            _type.type_adapter = _BaseModelTypeAdapter(_type)  # type: ignore
        else:
            ...
            object.__setattr__(_type, "type_adapter", TypeAdapter(_type))
