# ObjectSetWithPropertiesTypeDict

ObjectSet which returns objects with additional derived properties.

This feature is experimental and not yet generally available.


## Properties
| Name | Type | Required | Description |
| ------------ | ------------- | ------------- | ------------- |
**objectSet** | ObjectSetDict | Yes |  |
**derivedProperties** | typing.Dict[DerivedPropertyApiName, DerivedPropertyDefinitionDict] | Yes | Map of the name of the derived property to return and its definition  |
**type** | typing.Literal["withProperties"] | Yes | None |


[[Back to Model list]](../../../../README.md#models-v2-link) [[Back to API list]](../../../../README.md#apis-v2-link) [[Back to README]](../../../../README.md)
