...
[**apply_batch_with_overrides**](#apply_batch_with_overrides) | **POST** /v2/ontologies/{ontology}/actions/{action}/applyBatchWithOverrides | Private Beta |
...
# ActionTypeApiName | The API name of the action to apply. To find the API name for your action, use the **List action types** endpoint or check the **Ontology Manager**.
action = "rename-employee"
# List[BatchApplyActionRequestItemWithOverrides]
requests = [
    {
        "parameters": {"id": 80060, "newName": "Anna Smith-Doe"},
        "overrides": {
...
try:
    api_response = client.ontologies.Action.apply_batch_with_overrides(
        ontology,
        action,
        requests=requests,
        branch=branch,
    ...
    print("The apply_batch_with_overrides response:\n")
    pprint(api_response)
except foundry_sdk.PalantirRPCException as e:
    print("HTTP error when calling Action.apply_batch_with_overrides: %s\n" % e)
